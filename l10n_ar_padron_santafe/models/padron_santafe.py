...
"""Modelos para el padrón mensual de alícuotas IIBB API (Santa Fe).

Diseño:

* `l10n_ar.padron.santafe.import` — un registro por **archivo importado**.
* `l10n_ar.padron.santafe.alicuota` — un registro por **(CUIT, vigencia)**.
...
factura saliente. Index sobre `(cuit, date_from, date_to)`. API típicamente
...
from ..lib import padron_santafe as santafe_lib

_logger = logging.getLogger(__name__)


class PadronAgipImport(models.Model):
    _name = "l10n_ar.padron.santafe.import"
    _description = "Padrón API Santa Fe — archivo importado"
    _order = "date_from desc, id desc"
    _rec_name = "display_name"

    name = fields.Char(
        required=True,
        default=lambda self: _("API Nuevo"),
        help="Identificador del padrón. Por convención: 'API YYYY-MM'.",
    )
    ...
    def _compute_display_name(self):
        for rec in self:
            if rec.date_from and rec.date_to:
                ...
                rec.display_name = rec.name or _("API")
    ...
    def action_parse_file(self):
        ...
        try:
            ...
            records = santafe_lib.parse_bytes(content, self.file_name or "")
        except Exception as e:
            raise UserError(_("Error parseando el padrón API: %s") % e)

        if not records:
            raise UserError(_(
                "El archivo no tiene registros válidos. Verificá que sea "
                "un padrón API (TXT con campos separados por ';')."
            ))

        # Borrar alicuotas previas si re-import.
        self.alicuota_ids.unlink()

        Alic = self.env["l10n_ar.padron.santafe.alicuota"]
        vals_list = []
        min_d = max_d = None
        for r in records:
            d = r.get("date_from")
    ...
    def action_view_alicuotas(self):
        ...
        return {
            ...
            "res_model": "l10n_ar.padron.santafe.alicuota",
            "view_mode": "list,form",
...
class PadronAgipAlicuota(models.Model):
    _name = "l10n_ar.padron.santafe.alicuota"
    _description = "Padrón API Santa Fe — Alícuota"
    _order = "cuit, date_from desc"

    import_id = fields.Many2one(
        "l10n_ar.padron.santafe.import",
        ondelete="cascade",
        required=True,
        index=True,
    )
    ...
    marca_alic = fields.Selection(
        selection=[
            ("S", "Mismo grupo"),
            ("N", "Cambió grupo"),
        ],
        string="Marca alícuota",
    )
    ...
    def find_for(self, cuit, target_date=None, company=None):
        """Busca la alícuota API vigente para `cuit` en `target_date`.
