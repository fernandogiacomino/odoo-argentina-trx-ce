...
"""Aplicación automática de Percepción IIBB Santa Fe en account.move.

Reusa el template oficial **"P. IIBB SF 0%"** de Odoo 19 l10n_ar y
los helpers compartidos de `l10n_ar_padron_base`.
"""
import logging
...
class AccountMove(models.Model):
    ...
    def _onchange_partner_l10n_ar_santafe(self):
        for move in self:
            if move.move_type not in ("out_invoice", "out_refund"):
                continue
            move._l10n_ar_apply_padron_santafe()

    def _l10n_ar_apply_padron_santafe(self):
        self.ensure_one()
        ...
        alic = self.env["l10n_ar.padron.santafe.alicuota"].sudo().find_for(
        ...
        if alic and alic.aliquot_perception > 0:
            ...
            if not tax:
                _logger.warning(
                    "No se encontró template '%s 0%%' en %s — no aplica padrón SF.",
