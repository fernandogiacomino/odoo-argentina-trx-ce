...
"""Aplicación automática de Percepción IIBB CABA en account.move.

Reusa el template oficial **"P. IIBB CABA 0%"** de Odoo 19 l10n_ar.
La lógica común está en `l10n_ar_padron_base` — acá solo el wiring
con el padrón AGIP.
"""
import logging
...
class AccountMove(models.Model):
    ...
    def _onchange_partner_l10n_ar_agip(self):
        for move in self:
            if move.move_type not in ("out_invoice", "out_refund"):
                continue
            move._l10n_ar_apply_padron_agip()

    def _l10n_ar_apply_padron_agip(self):
        """Aplica/quita el tax CABA según el padrón AGIP."""
        self.ensure_one()
        ...
        alic = self.env["l10n_ar.padron.agip.alicuota"].sudo().find_for(
        ...
        if alic and alic.aliquot_perception > 0:
            ...
            if not tax:
                _logger.warning(
                    "No se encontró template '%s 0%%' en %s — no aplica AGIP.",
    ...
    def _post(self, soft=True):
        for move in self.filtered(lambda m: m.move_type in ("out_invoice", "out_refund")):
            try:
                ...
                _logger.warning(
                    "No se pudo aplicar padrón AGIP en %s: %s", move.name, e,
                )
