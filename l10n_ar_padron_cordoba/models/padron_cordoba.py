...
"""Modelos para el padrón mensual de alícuotas IIBB Rentas (Córdoba).

Diseño:

* `l10n_ar.padron.cordoba.import` — un registro por **archivo importado**.
* `l10n_ar.padron.cordoba.alicuota` — un registro por **(CUIT, vigencia)**.
...
factura saliente. Index sobre `(cuit, date_from, date_to)`. Rentas típicamente
...
from ..lib import padron_cordoba as cordoba_lib

_logger = logging.getLogger(__name__)


class PadronAgipImport(models.Model):
    _name = "l10n_ar.padron.cordoba.import"
    _description = "Padrón Rentas Córdoba — archivo importado"
    _order = "date_from desc, id desc"
    _rec_name = "display_name"

    name = fields.Char(
        required=True,
        default=lambda self: _("Rentas Nuevo"),
        help="Identificador del padrón. Por convención: 'Rentas YYYY-MM'.",
    )
    ...
    alicuota_ids = fields.One2many(
        "l10n_ar.padron.cordoba.alicuota",
        "import_id",
        string="Alícuotas",
    )
    ...
    def _compute_display_name(self):
        for rec in self:
            if rec.date_from and rec.date_to:
                ...
                rec.display_name = rec.name or _("Rentas")
    ...
    def action_parse_file(self):
        ...
        try:
            ...
            records = cordoba_lib.parse_bytes(content, self.file_name or "")
        except Exception as e:
            raise UserError(_("Error parseando el padrón Rentas: %s") % e)

        if not records:
            raise UserError(_(
                "El archivo no tiene registros válidos. Verificá que sea "
                "un padrón Rentas (TXT con campos separados por ';')."
            ))

        # Borrar alicuotas previas si re-import.
        self.alicuota_ids.unlink()

        Alic = self.env["l10n_ar.padron.cordoba.alicuota"]
        vals_list = []
        min_d = max_d = None
        for r in records:
            d = r.get("date_from")
        ...
        return {
            ...
            "params": {
                "title": _("Padrón Rentas importado"),
    ...
    def action_view_alicuotas(self):
        ...
        return {
            ...
            "res_model": "l10n_ar.padron.cordoba.alicuota",
            "view_mode": "list,form",
...
class PadronAgipAlicuota(models.Model):
    _name = "l10n_ar.padron.cordoba.alicuota"
    _description = "Padrón Rentas Córdoba — Alícuota"
    _order = "cuit, date_from desc"

    import_id = fields.Many2one(
        "l10n_ar.padron.cordoba.import",
        ondelete="cascade",
        required=True,
        index=True,
    )
    ...
    def find_for(self, cuit, target_date=None, company=None):
        """Busca la alícuota Rentas vigente para `cuit` en `target_date`.
