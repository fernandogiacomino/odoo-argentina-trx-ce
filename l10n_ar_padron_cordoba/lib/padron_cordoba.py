...
"""Parser del padrón mensual de IIBB Rentas (Córdoba / Córdoba).

Rentas publica el padrón mensual en su portal:

    https://www.cordoba.gob.ar/agentes/agentes-de-recaudacion/ib-agentes-recaudacion/padrones/

El archivo viene como `.RAR` o `.ZIP` y dentro tiene un `.TXT` con
**campos separados por punto y coma** (no longitud fija como ARBA).

Layout (RG Rentas 296/2019, modificado por RG 352/2022):
...
# Cantidad mínima de columnas que esperamos. Rentas a veces agrega campos
...
class PadronAgipParseError(Exception):
    """Error parseando el padrón Rentas — file mal formado o layout cambió."""
